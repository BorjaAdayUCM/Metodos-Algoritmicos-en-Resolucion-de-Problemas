...
public:
    ...
    ConjuntosDisjuntos(int N) : nelems(N), ncjtos(nelems), id(nelems, -1), tamMaxConjunto(0) {}
    ...
    void unir(int p, int q) {
        ...
        if (id[i] < id[j]) { // i es la raíz del mayor árbol
            ...
            if (-id[i] > tamMaxConjunto) tamMaxConjunto = -id[i];
        } else {
            id[j] += id[i]; id[i] = int(j);
            if (-id[j] > tamMaxConjunto) tamMaxConjunto = -id[j];
        }
        --ncjtos;
    }

    int maxConjunto() {
        return tamMaxConjunto;
    }
