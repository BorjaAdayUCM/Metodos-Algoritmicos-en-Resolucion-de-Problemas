...
template <class Clave, class Valor, class Comparador = less<Clave>>
class map {
public:
   // parejas <clave, valor>
   struct clave_valor {
   ...
   // using clave_valor = pair<const Clave, Valor>;
protected:
...
public:
   ...
   vector<Clave> entre(Clave const& c1, Clave const& c2) {
       vector<Clave> list;
       entre(raiz, c1, c2, list);
       return list;
   }
   ...
   Valor const& at(Clave const& c) const {
      Link p = busca(c, raiz);
      if (p == nullptr)
         throw out_of_range("La clave no se puede consultar");
      return p->cv.valor;
   }
...
protected:
   ...
   void entre(Link const a, Clave const& c1, Clave const& c2, vector<Clave> & list) {
       if (a != nullptr) {
           if (menor(c1, a->cv.clave)) entre(a->iz, c1, c2, list);
           if (c1 <= a->cv.clave && a->cv.clave <= c2) list.push_back(a->cv.clave);
           if (menor(a->cv.clave, c2)) entre(a->dr, c1, c2, list);
       }
   }
   ...
   static void subirMenor(Link & a, Link b, Link padre) {
      if (b->iz != nullptr){
         ...
         delete a;
         a = b;
         a->altura = max(altura(a->iz), altura(a->dr)) + 1;
      }
   }
   
   // imprime el árbol (solo para depurar)
   static void print(ostream & o, int indent, Link r) {
      if (r != nullptr){
...
protected:
   ...
   public:
      Apuntado & operator*() const {
         if (act == nullptr)
            throw out_of_range("No hay elemento a consultar");
         return act->cv;
      }
   ...
   protected:
      friend class map;
      Link act;
      stack<Link> ancestros;  // antecesores no visitados
      ...
      void next() {
         if (act == nullptr) {
            throw out_of_range("El iterador no puede avanzar");
...
inline ostream& operator<<(ostream & o, map<Clave, Valor, Comparador> const& a) {
   a.print(o);
   return o;
}
