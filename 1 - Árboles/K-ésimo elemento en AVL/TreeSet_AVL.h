...
protected:
   ...
   struct TreeNode {
      ...
      TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
               int alt = 1, int tam = 1, int n = 1) : elem(e), iz(i), dr(d), altura(alt), tam_i(tam) {}
   };
...
protected:
   ...
   T const& kesimo(int k, Link a, int suma) const {
       if (a != nullptr) {
           int posicion = a->tam_i + suma;
           if (posicion == k) return a->elem;
           else if (posicion < k) return kesimo(k, a->dr, suma + a->tam_i);
           else return kesimo(k, a->iz, suma);
       }
   }
